...
const REGISTRY_AFTER = 'const wDn=[{id:"general",label:"General",icon:"settings-gear"},{id:"router",label:"Router",icon:"git-branch"},{id:"bots",label:"Bots",icon:"agents"},{id:"usage",label:"Usage & Billing",icon:"chart-bars"},{id:"beta",label:"Updates",icon:"cloud-download"}]';
const GENERAL_BEFORE = 'Q=x==="general"?a.jsx(Te,{children:a.jsx(Sa,{auth:t})}):null';
const GENERAL_AFTER = 'Q=x==="general"?a.jsx(Te,{children:a.jsx(Sa,{auth:t})}):x==="router"?a.jsx(RRouterPanel,{}):x==="bots"?a.jsx(RBotsPanel,{}):null';
...
function RBotsState(){const[s,e]=de.useState({bots:[],error:null,busy:!0});de.useEffect(()=>{let t=!0;window.desktop.agent.getBotRoster().then(n=>{t&&e({bots:Array.isArray(n?.bots)?n.bots:[],error:null,busy:!1})}).catch(n=>{t&&e(r=>({...r,error:String(n?.message??n),busy:!1}))});return()=>{t=!1}},[]);const t=async n=>{e(r=>({...r,busy:!0,error:null}));try{const r=await n();e({bots:Array.isArray(r?.bots)?r.bots:[],error:null,busy:!1})}catch(r){e(i=>({...i,busy:!1,error:String(r?.message??r)}))}};return[s,t]}
function RBotsPanel(){const[s,e]=RBotsState(),[t,n]=de.useState(""),[r,i]=de.useState(""),[o,l]=de.useState("");const c=async()=>{if(t.trim().length===0)return;await e(()=>window.desktop.agent.upsertBot({name:t.trim(),title:r.trim(),description:o.trim()}));n("");i("");l("")};return a.jsx(Te,{children:a.jsxs("div",{className:k("sand-settings-general","sand-9f619 sand-78zum5 sand-dt5ytf sand-3qzy4x"),children:[a.jsx(re,{title:"Specialist bots",children:a.jsxs("div",{children:[s.bots.filter(d=>!d.hidden).map((d,u)=>a.jsx(ie,{divided:u>0,description:"@"+d.slug+(d.title?(" · "+d.title):"")+(d.provider?(" · "+d.provider):""),label:d.name,variant:"card",children:a.jsxs("div",{className:"sand-9f619 sand-78zum5 sand-6s0dn4",children:[a.jsx(oe,{disabled:s.busy,onClick:()=>e(()=>window.desktop.agent.hideBot(d.id,!0)),shape:"rectangular",size:"sm",variant:"secondary",children:"Hide"}),a.jsx(oe,{disabled:s.busy,onClick:()=>e(()=>window.desktop.agent.deleteBot(d.id)),shape:"rectangular",size:"sm",variant:"secondary",children:"Delete"})]})},d.id)),s.bots.some(d=>d.hidden)?a.jsx(se,{as:"p",color:"secondary",size:"sm",children:s.bots.filter(d=>d.hidden).length+" hidden"}):null,s.bots.length===0?a.jsx(se,{as:"p",color:"secondary",size:"sm",children:"Create a named specialist with its own instructions and optional provider pin."}):null]})}),a.jsx(re,{title:"New bot",children:a.jsxs("div",{children:[a.jsx("input",{"aria-label":"Bot name",className:RRouterInputClass,disabled:s.busy,onChange:d=>n(d.currentTarget.value),placeholder:"Name",style:{fontSize:13,height:34,marginBottom:8,padding:"0 10px",width:360},value:t}),a.jsx("input",{"aria-label":"Bot title",className:RRouterInputClass,disabled:s.busy,onChange:d=>i(d.currentTarget.value),placeholder:"Title",style:{fontSize:13,height:34,marginBottom:8,padding:"0 10px",width:360},value:r}),a.jsx("textarea",{"aria-label":"Bot standing instructions",className:RRouterInputClass,disabled:s.busy,onChange:d=>l(d.currentTarget.value),placeholder:"Standing instructions",style:{fontSize:13,minHeight:72,padding:10,width:360},value:o}),a.jsx(oe,{disabled:s.busy||t.trim().length===0,onClick:c,shape:"rectangular",size:"sm",variant:"primary",children:"Create bot"})]})}),s.error?a.jsx(se,{as:"p",color:"red",size:"sm",children:s.error}):null]})})}
`;
...
export async function applyOriginalRendererRouterPatch({ stageRoot }) {
  ...
  const record = {
    ...
    chunks: changes,
    features: ["settings-router-provider", "settings-local-docker-vm", "usage-current-provider", "settings-bot-roster"],
    transformations: ["settings-registry", "router-panel", "usage-panel", "bots-panel"],
  };
