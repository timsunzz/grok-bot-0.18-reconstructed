...
export function createMainEdgeHandlers(deps: MainEdgeDeps): HandlerMap {
  const handlers: HandlerMap = {
    ...
    setComputerUseModel: (raw) => { const requested = req(raw).model; const model = requested === null ? null : parseAgentModel(requested, true); if (requested === null || model != null) { invoke(deps.agentPrefsStore, "setComputerUseModel", model ?? undefined); void deps.syncHostSettingsToBox({ computerUseModel: invoke(deps.agentPrefsStore, "getComputerUseModel") ?? null }).catch((error: unknown) => reportDesktopEdgeFailure("host-settings", "computer-use-model", error)); } return computerUseModel(deps); },
    ...
    // The box runs the routed turn, so a provider it never received is not the provider in
    // effect. Reporting success anyway left the settings page naming one provider while every
    // turn kept going to the previous one, so an unreachable box rolls the choice back the way
    // `setBoxRuntime` below does.
    setInferenceRouter: async (raw) => { const provider = req(raw).provider; invariant(isSandInferenceProvider(provider), "Unknown inference provider."); const previous = invoke(deps.settingsStore, "getInferenceProvider"); invoke(deps.settingsStore, "setInferenceProvider", provider); let settings: UnknownRecord | null; try { settings = await deps.syncHostSettingsToBox({ inferenceProvider: provider }); } catch (error) { reportDesktopEdgeFailure("host-settings", "inference-router", error); settings = null; } if (settings == null) { invoke(deps.settingsStore, "setInferenceProvider", isSandInferenceProvider(previous) ? previous : "cursor"); throw new SandHostSettingsUnreachableError("Couldn't reach the computer to change the inference provider."); } return { provider, usage: settings.inferenceRouterUsage ?? invoke(deps.settingsStore, "getInferenceRouterUsage") ?? null, local: getLocalInferenceCliStatus() }; },
    ...
    setWebauthnProxyEnabled: async (raw) => { const enabled = req(raw).enabled === true; invoke(deps.boxToggleStore, "setWebauthnProxyEnabled", enabled); deps.emitWebauthnProxyChanged(enabled); const mirrored = sandWebauthnProxyMirroredEnablement(enabled, deps.platform); for (let attempt = 0; attempt < 3; attempt += 1) { try { const applied = await deps.syncHostSettingsToBox({ webauthnProxyEnabled: mirrored }); if (applied?.webauthnProxyEnabled === mirrored) break; } catch (error) { reportDesktopEdgeFailure("host-settings", "webauthn-proxy-retry", error); } await (deps.delay ?? sleep)(250 * (attempt + 1)); } return invoke(deps.boxToggleStore, "getWebauthnProxyEnabled"); },
    getOnboardingSeen: async () => await Promise.resolve(invoke(deps.onboardingSeen, "reconcile")) === true,
    setOnboardingSeen: (raw) => { const seen = req(raw).seen; if (typeof seen === "boolean") void Promise.resolve(invoke(deps.onboardingSeen, "apply", seen)).catch((error: unknown) => reportDesktopEdgeFailure("onboarding", "apply-seen", error)); },
