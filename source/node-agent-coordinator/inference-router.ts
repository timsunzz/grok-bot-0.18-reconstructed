...
type Store = {
  ...
  readonly agents: Readonly<Record<string, readonly StoredEntry[]>>;
};
...
export function parseInferenceRouterTranscriptStore(value: unknown): Store {
  ...
  if (root?.schemaVersion !== 2 || asRecord(root.agents) == null)
    return EMPTY_STORE;
  const agents: Record<string, StoredEntry[]> = {};
  for (const [agentId, rawEntries] of Object.entries(
    ...
    for (const raw of rawEntries) {
      ...
      if (
        row == null ||
        !["codex", "claude-code", "openrouter"].includes(
          String(row.provider),
        ) ||
        ...
        (row.clientNonce !== undefined &&
          typeof row.clientNonce !== "string") ||
      ...
      if (
        ...
        (!Array.isArray(row.reactions) ||
          row.reactions.some(
            (reaction) =>
              asRecord(reaction) == null ||
              typeof asRecord(reaction)!.emoji !== "string" ||
              typeof asRecord(reaction)!.by !== "string",
          ))
      )
        continue;
...
export function projectInferenceRouterTranscriptEntry(
  entry: StoredEntry,
...
export function mergeInferenceRouterTranscriptEntries(
  ...
  for (let index = remote.length + local.length - 1; index >= 0; index -= 1) {
    const entry =
      index < remote.length ? remote[index] : local[index - remote.length];
...
export function createCoordinatorInferenceRouter(options: {
  ...
  const settings = new SandSettingsStore(
  ...
  const load = async (): Promise<Store> => {
    try {
      return parseInferenceRouterTranscriptStore(
  ...
  const append = async (
    ...
    const next: Store = {
      ...
      agents: {
        ...current.agents,
        [agentId]: [...(current.agents[agentId] ?? []), ...entries].slice(-200),
      },
    };
    await persist(next);
    return next;
      ...
      const project = (isRunning: boolean) =>
        remote.map((raw) => {
          ...
          return {
            ...row,
            isRunning,
            isRunningTurn: isRunning,
            isComposingMessage: isRunning,
            isRetrying: false,
            ...(isRunning
  ...
  const toggleLocalReaction = async (
    agentId: string,
    entryId: string,
    emoji: string,
  ): Promise<Record<string, unknown> | null> => {
    const trimmed = emoji.trim();
    if (agentId.length === 0 || entryId.length === 0 || trimmed.length === 0)
      return null;
    const current = await load();
    ...
    const exists = reactions.some(
      (reaction) => reaction.emoji === trimmed && reaction.by === "me",
    );
    const nextReactions = exists
      ? reactions.filter(
          (reaction) => !(reaction.emoji === trimmed && reaction.by === "me"),
        )
      : [...reactions, { emoji: trimmed, by: "me" }];
    const { reactions: _oldReactions, ...withoutReactions } = before;
    const updated: StoredEntry =
      nextReactions.length === 0
        ? withoutReactions
        : { ...withoutReactions, reactions: nextReactions };
    const nextEntries = [...entries];
    nextEntries[index] = updated;
    await persist({
      schemaVersion: 2,
      agents: { ...current.agents, [agentId]: nextEntries },
    });
  ...
  const execute = async (
    ...
    const richText =
      typeof args.richText === "string" ? args.richText : undefined;
    const clientNonce =
      typeof args.clientNonce === "string" ? args.clientNonce : randomUUID();
    ...
    const [remote, beforeUser] = await Promise.all([
      options.dispatchRemote("getAgentTranscriptTail", { id: agentId }),
      load(),
    ]);
    ...
    const remoteTurn = remoteEntries.reduce<number>((highest, raw) => {
      ...
      const match =
        typeof id === "string" ? /^t(\d+)(?:u|s\d+)$/.exec(id) : null;
      return match == null ? highest : Math.max(highest, Number(match[1]));
    }, -1);
    const localTurn = (beforeUser.agents[agentId] ?? []).reduce(
      (highest, entry) => {
    ...
    const messages = (withUser.agents[agentId] ?? []).map((entry) => ({
      role: entry.role,
      content: entry.content,
    }));
    let content: string;
    ...
    const emitAssistant = (nextContent: string, streaming: boolean) => {
      const entry = {
        kind: "send-message",
        id: assistantId,
        message: { type: "text", content: nextContent },
        streaming,
        timestampMs: assistantTimestampMs,
      };
      emitTranscript(
        agentId,
        assistantStreamStarted ? "updated" : "appended",
        entry,
      );
    ...
    const bridge =
      provider === "claude-code"
        ? await createRoutedMcpBridge({
            listTools: () => options.dispatchRemote("listRoutedMcpTools", {}),
            callTool: (tool) =>
              options.dispatchRemote("executeRoutedMcpTool", {
                ...tool,
                agentId,
              }),
          })
    ...
    const directTools =
      bridge == null
        ? await options.dispatchRemote("listRoutedMcpTools", {})
        : undefined;
    const tools = Array.isArray(directTools)
      ? (directTools as Record<string, any>[])
      : undefined;
    const onTextDelta = (_delta: string, accumulated: string) =>
      emitAssistant(accumulated, true);
    try {
      content = await runRoutedProviderText(
        provider,
        messages,
        bridge == null
          ? {
  ...
  return {
    provider(): SandInferenceProvider {
      return settings.getInferenceProvider();
    },
    async dispatch(
      method: string,
      args: unknown,
      ...
      if (
        ...
        return {
          handled: true,
          value: { ...result, entries: entries.slice(-limit) },
        };
      }
      if (method !== "sendPrompt" || provider === "cursor")
      ...
      const next = previous
        ...
        .catch(async (error) => {
          ...
          if (agentId.length > 0) {
            ...
            await append(agentId, [
              { provider, role: "assistant", content, id, timestampMs },
            ]);
      ...
      return {
        handled: true,
        value: { accepted: true, clientNonce: record.clientNonce, provider },
      };
    },
  };
}
