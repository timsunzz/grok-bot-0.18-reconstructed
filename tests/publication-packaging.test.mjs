...
test("Router settings use the trusted backend and display recorded inference usage", async () => {
  ...
  assert.match(coordinator, /withRoutedTurnDeadline/);
  assert.match(coordinator, /classifyRoutedProviderError/);
  assert.match(coordinator, /accepted: false/);
  assert.match(coordinator, /inFlightNonces/);
