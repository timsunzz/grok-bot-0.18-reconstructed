...
test("gateway JSON parsing rejects malformed request bodies as client errors", async () => {
  const protocol = await loadModule("source/host/gateway-protocol.ts", "gateway-protocol");
  const server = await loadModule("source/host/gateway-server.ts", "gateway-server");
  try {
    assert.deepEqual(protocol.module.parseCommandArgs(""), {});
    assert.deepEqual(protocol.module.parseCommandArgs('{"ok":true}'), { ok: true });
    assert.throws(
    ...
    assert.equal(
      server.module.statusForCommandError(new server.module.SandGatewayRequestError("invalid")),
      400,
    );
  } finally {
